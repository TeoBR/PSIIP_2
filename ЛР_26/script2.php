...
echo "<p style='font-weight: bold;'>Марка: " . $row["brand"]. "</p>";
echo "<p>Модель: " . $row["model"]. "</p>";
echo "<p>Размер: " . $row["size"]. "</p>";
echo "<p>Разрешение: " . $row["resolution"]. "</p>";
echo "<p>Цена: $" . $row["price"]. "</p>";
echo "<a href='?action=add_to_cart&product_id=" . $row['id'] . "' style='display: block; background-color: #4CAF50; color: white; text-align: center; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>Добавить в корзину</a>";
echo "</div>";
    }
} else {
    echo "0 результатов";
...
for ($i = 1; $i <= $total_pages; $i++) {
    echo "<a href='?page=$i' style='display: inline-block; padding: 8px 12px; margin-right: 5px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px;'>$i</a>";
}
echo "</div>";

// Форма для поиска
echo "<form method='get' style='margin-top: 20px;'>";
echo "<input type='text' name='search' placeholder='Поиск по марке или модели' style='padding: 8px; border-radius: 5px;'>";
echo "<input type='submit' value='Искать' style='background-color: #4CAF50; color: white; padding: 8px 12px; border: none; border-radius: 5px; cursor: pointer; margin-left: 10px;'>";
echo "</form>";

// Форма для сортировки
echo "<form method='get' style='margin-top: 20px;'>";
echo "<label for='sort_column' style='margin-right: 10px;'>Сортировать по:</label>";
echo "<select name='sort_column' style='margin-right: 10px; padding: 8px; border-radius: 5px;'>";
...
echo "</select>";
echo "<select name='sort_order' style='margin-right: 10px; padding: 8px; border-radius: 5px;'>";
echo "<option value='ASC'>По возрастанию</option>";
echo "<option value='DESC'>По убыванию</option>";
echo "</select>";
echo "<input type='submit' value='Сортировать' style='background-color: #4CAF50; color: white; padding: 8px 12px; border: none; border-radius: 5px; cursor: pointer;'>";
echo "</form>";

// Форма для фильтрации
echo "<form method='get' style='margin-top: 20px;'>";
echo "<select name='filter' style='padding: 8px; border-radius: 5px;'>";
...
echo "</select>";
echo "<input type='submit' value='Применить фильтр' style='background-color: #008CBA; color: white; padding: 8px 12px; border: none; border-radius: 5px; cursor: pointer; margin-left: 10px; '>";
echo "</form>";

// Ссылка на страницу корзины
echo "<a href='cart.php' style='display: inline-block; margin-top: 20px; padding: 10px 20px; background-color: #008CBA; color: white; text-decoration: none; border-radius: 5px;'>Перейти в корзину</a>";
